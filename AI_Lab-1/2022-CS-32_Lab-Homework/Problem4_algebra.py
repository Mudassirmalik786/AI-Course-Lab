...
def do_multiply(expr1, expr2):
    # If both expressions are numbers (int or float), return their product directly
    if isinstance(expr1, (int, float)) and isinstance(expr2, (int, float)):
        return Product([expr1, expr2])

    # If one of the expressions is a symbol (str), return a product of the expressions
    if isinstance(expr1, str) or isinstance(expr2, str):
        return Product([expr1, expr2])

    # Case 1: Both expressions are sums
    if isinstance(expr1, Sum) and isinstance(expr2, Sum):
        terms = [do_multiply(term1, term2) for term1 in expr1 for term2 in expr2]
        return Sum(terms).simplify()

    # Case 2: expr1 is a sum and expr2 is a product
    elif isinstance(expr1, Sum) and isinstance(expr2, Product):
        terms = [do_multiply(term1, expr2) for term1 in expr1]
        return Sum(terms).simplify()

    # Case 3: expr1 is a product and expr2 is a sum
    elif isinstance(expr1, Product) and isinstance(expr2, Sum):
        terms = [do_multiply(expr1, term2) for term2 in expr2]
        return Sum(terms).simplify()

    # Case 4: Both expressions are products
    elif isinstance(expr1, Product) and isinstance(expr2, Product):
        return Product(list(expr1) + list(expr2)).simplify()

    # Handle cases where one or both expressions are basic types
    elif isinstance(expr1, (int, float, str)):
        return Product([expr1, expr2]).simplify()

    elif isinstance(expr2, (int, float, str)):
        return Product([expr1, expr2]).simplify()

    else:
        raise ValueError("Invalid expression types for multiplication")
